...
Ollama(로컬 LLM), Gemini API, OpenRouter, Claude Agent SDK를 지원하며, 여러 앱(DeepAssist, TestMancer 등)을 하나의 플랫폼에서 전환하여 사용할 수 있습니다.
...
- **`apps/`**: 앱 레지스트리 + 앱별 독립 모듈
  ...
  - **`apps/deep_assist/`**: DeepAssist (자율 코딩 에이전트)
    ...
    - `page.py`: `core/` 유틸리티를 재사용. 사이드바(LLM 4종 프로바이더), 메인(채팅+워크스페이스 탭)
...
- **`core/`**: 공유 유틸리티 (강제 아님, 앱이 선택적으로 import)
  ...
  - **`core/sidebar.py`**: LLM 프로바이더 선택 UI. `render_llm_sidebar()` → 설정 dict 반환. 앱이 사이드바를 자체 구현할 경우 사용하지 않아도 됨
